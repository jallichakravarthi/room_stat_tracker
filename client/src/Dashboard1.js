...
function Dashboard1() {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (isFullscreen && (window.innerWidth < 1200 || window.innerHeight < 800)) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const latestResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/room1`);
        ...
        if (token && hours > 6) {
          ...
          try {
            const historyResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/room1/history?hours=${hours}`, { headers });
            if (historyResponse.status === 401 || historyResponse.status === 403) {
              console.error('[Dashboard1] 401/403 on /history fetch');
              alert('Session expired or unauthorized. Please log in.');
              navigate('/login');
              return;
            }
            ...
            console.log('[Dashboard1] /history data:', historyData);
            chartLabels = historyData.map(d => format(new Date(d.timestamp), 'HH:mm'));
            tempData = historyData.map(d => d.temperature);
            humidityData = historyData.map(d => d.humidity);
          } catch (err) {
            console.error('[Dashboard1] Error fetching /history:', err);
          ...
          const recentResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/room1/recent-history`);
          ...
          const latestResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/room1`);
          if (latestResponse.status === 401 || latestResponse.status === 403) {
            console.error('[Dashboard1] 401/403 on guest fetch');
            alert('Session expired or unauthorized. Please log in.');
            navigate('/login');
            return;
          }
          ...
          console.log('[Dashboard1] /api/room1 guest data:', allData);
          const timeAgo = subHours(new Date(), hours);
          const filtered = allData.filter(d => new Date(d.timestamp) > timeAgo);
          console.log('[Dashboard1] Filtered guest data:', filtered);
          chartLabels = filtered.map(d => format(new Date(d.timestamp), 'HH:mm'));
        ...
        console.log('[Dashboard1] Updated chartData:', { labels: chartLabels, tempData, humidityData });
      } catch (error) {
        console.error('[Dashboard1] Error fetching sensor data:', error);
  ...
  return (
    <Layout isFullscreen={isFullscreen} onToggleFullscreen={isLargeScreen ? toggleFullscreen : null}>
      <div className="dashboard-container animate-fadeIn">
        ...
        <div className="w-full flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-4 py-4 bg-gradient-to-r from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 shadow-lg border-b border-gray-200 dark:border-gray-700 mb-7">
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Room 1 Sensor Dashboard</h1>
            {isLive ? (
              <div className="flex items-center mt-2 md:mt-0">
                ...
                <span className="text-gray-600 dark:text-gray-300">Next update in {countdown}s</span>
              </div>
            ) : (
              <div className="flex items-center mt-2 md:mt-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {/* Temperature Card */}
          <div className="sensor-item p-6 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl animate-fadeIn" style={{ animationDelay: '0.1s' }}>
            ...
            <div className="flex items-end justify-between">
              <div>
                <div className={`text-3xl font-bold ${getTempStatusClass(latestData?.temperature)}`}>
                  {latestData?.temperature ? latestData.temperature.toFixed(1) : 'N/A'}
                  <span className="text-xl">°C</span>
                </div>
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {latestData?.temperature && (
                    latestData.temperature >= 30 || latestData.temperature <= 18 ? 
                    <span className="text-status-critical">Critical</span> :
                    latestData.temperature >= 28 || latestData.temperature <= 20 ? 
          ...
          {/* Humidity Card */}
          <div className="sensor-item p-6 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl animate-fadeIn" style={{ animationDelay: '0.2s' }}>
            <h3 className="text-lg font-semibold mb-4 flex items-center">
            ...
            <div className="flex items-end justify-between">
              <div>
                <div className={`text-3xl font-bold ${getHumidityStatusClass(latestData?.humidity)}`}>
                  {latestData?.humidity ? latestData.humidity.toFixed(1) : 'N/A'}
                ...
                <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {latestData?.humidity && (
                    ...
                    <span className="text-status-critical">Critical</span> :
                    latestData.humidity >= 70 || latestData.humidity <= 30 ? 
                    <span className="text-status-warning">Warning</span> :
                    <span className="text-status-good">Normal</span>
                  )}
                </div>
          ...
          {/* Air Quality Card */}
          <div className="sensor-item p-6 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl animate-fadeIn" style={{ animationDelay: '0.3s' }}>
            <h3 className="text-lg font-semibold mb-4 flex items-center">
            ...
            <div className="flex items-end justify-between">
              <div>
                <div className="text-3xl font-bold text-green-600 dark:text-green-400">
                  {latestData?.mq135?.co2 ? Math.round(latestData.mq135.co2) : 'N/A'}
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">NH₃</div>
              <div className="font-medium">
                {latestData?.mq135?.nh3 ? latestData.mq135.nh3.toFixed(1) : 'N/A'} ppm
              </div>
            </div>
          </div>

          {/* Gas Sensors Card */}
          <div className="sensor-item p-6 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 transition-all duration-300 transform hover:scale-[1.02] hover:shadow-xl animate-fadeIn" style={{ animationDelay: '0.4s' }}>
            <h3 className="text-lg font-semibold mb-4 flex items-center">
              <svg className="w-5 h-5 text-red-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-1">
                  <span className="sensor-label font-medium text-gray-700 dark:text-gray-300">CO</span>
              ...
              <div>
                <div className="flex justify-between mb-1">
                  <span className="sensor-label font-medium text-gray-700 dark:text-gray-300">CH₄</span>
              ...
              <div>
                <div className="flex justify-between mb-1">
                  <span className="sensor-label font-medium text-gray-700 dark:text-gray-300">LPG</span>
        ...
        <div className="mb-8 bg-white dark:bg-dashboard-card-dark p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold mb-2">Temperature & Humidity Trends</h2>
              <p className="text-gray-600 dark:text-gray-300">
                Average values over 30-minute intervals for the last {hours} hours
              </p>
            </div>
            <div className="flex space-x-2 mt-2 md:mt-0">
              {[6, 12, 24, 48].map((h) => (
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-full transition-all duration-300 ${hours === h ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600'} ${!localStorage.getItem('token') && h !== 6 ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {h}h
                </button>
              ))}
          ...
          {latestData ? (
            <div style={{ height: isFullscreen ? '300px' : '300px', position: 'relative' }}>
              <Line 
                data={chartData} 
                options={{
                  responsive: true,
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              ...
              <p className="text-sm">Looking for data from the last 12 seconds</p>
            </div>
          )}
        </div>

        {/* Recent Data Table */}
        <div className={`table-container bg-white dark:bg-dashboard-card-dark p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 ${isFullscreen ? 'max-h-60 overflow-y-auto' : ''}`}>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead>
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Humidity</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">MQ9 Sensors</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">MQ135 Sensors</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {recentData.slice(0, isFullscreen ? 5 : undefined).map((data, index) => (
                  <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                      {data.mq9
                        ? `CO: ${data.mq9.co?.toFixed(2) ?? "N/A"} | CH₄: ${data.mq9.ch4?.toFixed(2) ?? "N/A"} | LPG: ${data.mq9.lpg?.toFixed(2) ?? "N/A"}`
                        : "N/A"}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                      {data.mq135
                        ? `CO₂: ${data.mq135.co2?.toFixed(2) ?? "N/A"} | NH₃: ${data.mq135.nh3?.toFixed(2) ?? "N/A"}`
                        : "N/A"}
                    </td>
                  </tr>
                ))}
